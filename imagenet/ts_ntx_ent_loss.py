...
class TSNTXentLoss(MemoryBankModule):
    ...
    def __init__(
        ...
        super(TSNTXentLoss, self).__init__(size=memory_bank_size)
        ...
        if abs(self.max_temp) < self.eps:
            raise ValueError(
                "Illegal temperature: abs({}) < 1e-8".format(self.max_temp)
            )
    ...
    def get_temp(self, cossim):
        temp = self.min_temp + 0.5*(self.max_temp - self.min_temp)*(1+torch.cos((1 + cossim)*torch.tensor(np.pi)))
        return temp
    
    def get_temp2(self, cossim):
        offset = -0.4
        scale = 0.7
        ...
        if offset <= 0:
            temp[cossim <= -offset] = self.min_temp + 0.5*(self.max_temp - self.min_temp)*(1+torch.cos((offset+cossim[cossim<=-offset])*torch.tensor(np.pi)/scale))
            temp[cossim > -offset] = self.max_temp #a - b * torch.exp(-c*cossim[cossim>0.0])
        else:
            temp[cossim >= -offset] = self.min_temp + 0.5*(self.max_temp - self.min_temp)*(1+torch.cos((offset+cossim[cossim<=-offset])*torch.tensor(np.pi)/scale))
            temp[cossim < -offset] = self.max_temp #a - b * torch.exp(-c*cossim[cossim>0.0])
    ...
    def forward(self, out0: torch.Tensor, out1: torch.Tensor):
        ...
        out1, negatives = super(TSNTXentLoss, self).forward(
        ...
        if negatives is not None:
            ...
            logits = logits / self.get_temp(logits.detach())
